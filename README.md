...
1.0 正式版。单管理员自用的销售项目全链路管理专家系统，覆盖商机从客户信息到项目闭环的完整生命周期。

## 1.0 正式版

- 业务数据已清空，适合正式启用初始化
- 订单编号规则：ID 为数据库自增 id 的 4 位补零（如 `0001`），前台展示与数据库完全一致
- 报价编号规则：`Q-客户简称-日期-R轮次`
- 支持最终客户/合同客户简称
- 商机进行中 / 存档双列表，闭环订单自动归档
...
冒烟测试覆盖：登录、价格决策、商机创建、报价提交、双线审批、中标、财务、发货批次校验、超开票审计、佣金幂等匹配、审批驳回重提、人工补录、数据修正回退、软/硬重置与 JWT 轮换。
...
为每个流程阶段生成 2 条测试订单，为已到达的每个可上传附件步骤生成 2 条真实可下载的附件记录（客户信息/方案/财务/开票），并模拟 10 条指导价与 10 条框架协议价。重复执行会先替换旧测试数据（`[测试]` 标记订单与 `TEST-` 价格）；`pnpm --filter iproject-server seed:test -- --clean` 会额外清理 `[测试]` 客户。
