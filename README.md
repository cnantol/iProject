...
为每个流程阶段生成 2 条测试订单，为已到达的每个可上传附件步骤生成 2 条真实可下载的附件记录（客户信息/方案/财务/开票），并模拟 10 条指导价与 10 条框架协议价。重复执行会先替换旧测试数据（`OPP-TEST-` 前缀订单与 `TEST-` 价格）；`pnpm --filter iproject-server seed:test -- --clean` 会额外清理 `[测试]` 客户。
