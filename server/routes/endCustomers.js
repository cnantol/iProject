...
router.post('/', (req, res) => {
  ...
  try {
    const info = getDb()
      .prepare('INSERT INTO end_customers (customer_name, short_name, contact_person, phone, email, remark, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)')
...
router.put('/:id', (req, res) => {
  ...
  try {
    getDb()
      .prepare('UPDATE end_customers SET customer_name=?, short_name=?, contact_person=?, phone=?, email=?, remark=?, updated_at=? WHERE id=?')
      .run(
...
router.delete('/:id', (req, res) => {
  ...
  try {
    ...
    if (String(err.message).includes('FOREIGN KEY')) return badRequest(res, '该客户已被销售机会或框架协议引用，无法删除');
    throw err;
  }
});
