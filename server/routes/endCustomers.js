...
router.post('/', (req, res) => {
  ...
  try {
    const info = getDb()
      .prepare('INSERT INTO end_customers (customer_name, contact_person, phone, email, remark, created_at, updated_at) VALUES (?,?,?,?,?,?,?)')
...
router.put('/:id', (req, res) => {
  ...
  try {
    getDb()
      .prepare('UPDATE end_customers SET customer_name=?, contact_person=?, phone=?, email=?, remark=?, updated_at=? WHERE id=?')
      .run(
