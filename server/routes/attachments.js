...
attachmentAdminRouter.get('/', (req, res) => {
  ...
  const itemsWithSize = items.map((item) => {
    let size = 0;
    try {
      size = fs.statSync(path.join(getUploadDir(), item.file_path)).size;
    } catch {
      size = 0;
    }
    return { ...item, size };
...
attachmentAdminRouter.get('/stats', (req, res) => {
  ...
  for (const row of all) {
    let size = 0;
    try {
      size = fs.statSync(path.join(getUploadDir(), row.file_path)).size;
    } catch {
      size = 0;
    }
    totalSize += size;
    ...
    if (found) {
      ...
      byType.push({ file_type: row.file_type, count: 1, size });
    }
  }
